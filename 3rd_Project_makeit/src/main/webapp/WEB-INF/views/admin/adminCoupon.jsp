	...
	<div id="layoutSidenav_content">
		<main>
			<div class="container-fluid px-4">
				<h1 class="mt-4">스토어 쿠폰 발급</h1>
				<ol class="breadcrumb mb-4">
					<li class="breadcrumb-item active">스토어 상품에 대한 쿠폰을 발급할 수 있습니다</li>
				</ol>
					...
					<div class="row">
						<div id="back1" class="col-md-3">
							<div class="contact-info">
								...
								<h2>Store Coupon</h2>
								<h4>스토어 쿠폰을 발급해보세요 !</h4>
							</div>
						</div>
						<div id="back" class="col-md-9">
							<form action="insertCoupon.do">
							<div class="contact-form">
								...
								<div class="form-group">
								  ...
								  <div class="col-sm-10">
									<select class="form-select" aria-label="Default select example" id="selectBox" name="storeIdx">
									  <c:if test="${ empty couponStore }">
										  <option value="" selected>등록된 상품이 없습니다</option>
									  </c:if>
									  <c:if test="${ not empty couponStore }">
									  	<option value="" selected disabled hidden>적용할 상품을 선택하세요</option>
									  	<c:forEach var="store" items="${couponStore }">
									  		<option value="${store.storeIdx }">${store.storeTitle }</option>
									  	</c:forEach>
									  </c:if>
									</select>
								  </div>
