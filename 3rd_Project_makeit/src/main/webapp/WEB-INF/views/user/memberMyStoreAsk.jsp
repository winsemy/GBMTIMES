			...
			<div class="row h-100 align-items-center">
				<div class="col-12">
					...
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">My Store Ask</li>
					</ol>
				</div>
			</div>
		</div>
                            ...
                            <table class="table table-bordered mb-30">
                                ...
                                <tbody>
                                	...
                                	<c:forEach var="ask" items="${MyAsk }">
	                                    <tr>
	                                        <td>
	                                            <a href="myQnaView.do?qnaIdx=${ask.qnaIdx }">${ask.qnaTitle }</a>
	                                        </td>
	                                        <td>${ask.qnaDate }</td>
	                                        <td>
		                                        <c:if test="${ask.qnaChange == 0 }">
		                                        	미응답
		                                        </c:if>
		                                        <c:if test="${ask.qnaChange == 1 }">
		                                        	답변완료
		                                        </c:if>
	                                        </td>
	                                        <td>${ask.qnaCategory }</td>
	                                    </tr>
	                                </c:forEach>
                                    </c:if>
                                </tbody>
									...
									<div class="shop_pagination_area mt-30">
										<nav aria-label="Page navigation">
											<ul class="pagination pagination-sm justify-content-center"  style="display: flex;" id="ajaxpageinsert" >
												...
												<c:if test="${page.beginPage!=1 }">
													<li class="page-item">
														<a class="page-link" href="memberMyStoreAsk.do?cPage=${page.beginPage-1}&memberId=${user.memberId}"><i class="fa fa-angle-left" aria-hidden="true"></i></a>
													</li>
												</c:if>
												...
												<c:forEach var="pageNo" begin="${page.beginPage }" end="${page.endPage }">
													...
													<c:if test="${pageNo != page.nowPage }">
														<li class="page-item">
															<a class="page-link" href="memberMyStoreAsk.do?cPage=${pageNo }&memberId=${user.memberId}">${pageNo }</a>
	 													</li>
													</c:if>
												</c:forEach>
												...
												<li class="page-item"> 
													<a class="page-link" href="memberMyStoreAsk.do?cPage=${page.endPage +1 }&memberId=${user.memberId}"><i class="fa fa-angle-right" aria-hidden="true"></i></a>
												</li>
												</c:if>
												<c:if test="${page.endPage >= page.totalPage  }">
													<li class="page-item">
