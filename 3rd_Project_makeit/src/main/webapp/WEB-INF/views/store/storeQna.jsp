                     ...
                     <div class="row">
	                     <div class="col-md-6">
		                     <div class="go_productDetailView">
		                     	<i class="fa fa-arrow-left"><a href="detail.do?storeIdx=${vo.storeIdx }&memberId=${memberId}" style="color: white">&nbsp;  상세페이지로 이동</a></i>
		                     </div>
	                     </div>
						<div class="col-md-6" >
							<div class="form-group">
								<form action="QnaInsertForm.do" style="margin: 0 auto;text-align: right;float:right">
									...
									<input type="hidden" value="${vo.storeIdx }" name="storeIdx">
								</form>
	                                	...
	                                	<tr> 
	                                        ...
	                                        <c:choose>
		                                        <c:when test="${list.qnaChange == 0 }">
			                                        <c:choose>
				                                        <c:when test="${fn:length(list.qnaTitle) >15 }">
				                                        	 <th scope="row" width="55%"><a href="qnaView.do?qnaIdx=${list.qnaIdx }&memberId=${memberId}&storeIdx=${vo.storeIdx}">${fn:substring(list.qnaTitle,0,15)}...</a></th>
				                                        </c:when>
				                                        <c:otherwise>
				                                        	<th scope="row" width="55%"><a href="qnaView.do?qnaIdx=${list.qnaIdx }&memberId=${memberId}&storeIdx=${vo.storeIdx}">${fn:substring(list.qnaTitle,0,15)}</a></th>
				                                        </c:otherwise>
		                                        ...
		                                        <c:otherwise>
		                                        	<c:choose>
				                                        <c:when test="${fn:length(list.qnaTitle) >15 }">
				                                        	 <th scope="row" width="55%"><a href="qnaView.do?qnaIdx=${list.qnaIdx }&memberId=${memberId}&storeIdx=${vo.storeIdx}">${fn:substring(list.qnaTitle,0,15)}...<b style="color:red;">Re : </b>[ ${fundingQnaList.statusChange } ]</a></th>
				                                        </c:when>
				                                        <c:otherwise>
				                                        	<th scope="row" width="55%"><a href="qnaView.do?qnaIdx=${list.qnaIdx }&memberId=${memberId}&storeIdx=${vo.storeIdx}">${fn:substring(list.qnaTitle,0,15)} <b style="color:red;">Re : </b>[ ${fundingQnaList.statusChange } ]</a></th>
				                                        </c:otherwise>
