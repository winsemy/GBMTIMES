			...
			if(category=="STORE"){
				$.each(data.feedlist,function(index,list){
					...
					if(list.storeReviewImg == null){
						mainFeedHtml += "<img class='normal_img' src='imgs/"+list.storeImg+"' alt='메인이미지'></div>";
					}else{
						mainFeedHtml += "<img class='normal_img' src='imgs/"+list.storeReviewImg+"' alt='메인이미지'></div>";
					}
				...
				$.each(data.feedlist,function(index,list){
					...
					if(list.fundReviewImg ==  null || list.fundReviewImg ==''){
						mainFeedHtml += "<img class='normal_img' src=\"img/fundingMainImg/"+list.fundMainImg+"\" alt='사진없음' style='height: 100%'></div>";
					}else{
						mainFeedHtml += "<img class='normal_img' src=\"img/fundingMainImg/"+list.fundReviewImg+"\" alt='메인이미지' style='height: 100%'></div>";
					}
					mainFeedHtml += "<div class='product_description'>";
		...
		<a href="productDetailView.do?memberId=${memberId}&fundIdx=${blist.fundIdx}" data-animation="fadeInUp">
			<div class="single_slide bg-img" style="background-image: url(img/fundingMainImg/${blist.fundMainImg}); background-size:cover;">
				<div class="container h-100" >
					<div>
						<p style="animation:shake 2s infinite; font-size: 3.5em;">special</p>
						<h2 data-animation="fadeInUp" data-delay="500ms" style="font-size: 55px;font-weight: 600; text-align: center; color: white;">${blist.fundTitle }</h2>
					</div>
					<h4 data-animation="fadeInUp" data-delay="700ms" style="color: #dc3545;float: right; font-family: 'GangwonEdu_OTFBoldA'; font-size: xx-large;"></h4>
				</div>
			</div>
		</a>
	</c:forEach>
						...
						<div class="product_image">
							...
							<c:choose>
								<c:when test="${flist.fundReviewImg eq null || flist.fundReviewImg eq '' }">
									<img class="normal_img" src="img/fundingMainImg/${flist.fundMainImg }" alt="메인이미지" style="height: 100%">
								</c:when>
								<c:otherwise>
									<img class="normal_img" src="img/fundingMainImg/${flist.fundReviewImg }" alt="사진없음">
								</c:otherwise>
							</c:choose>
						</div>
						<!-- Product Description -->
				...
				<div class="main_follow_2">
					...
					<c:if test="${memberId ne '' && followfundlist ne null && not empty followfundlist }">
						...
						<div id="follow_content_1">					
							<div class="follow_1">
								<button id="follow_2" onclick="location.href='follow.do'">팔로우 추가</button>
							</div>
						</div>
					</c:if>
					<!-- 일반 또는 리스트가 없을 경우 -->
