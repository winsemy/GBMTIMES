				...
				for(var i=0; i<FundEndNum; i++){
					if(flist.length> FundEndNum || flist[i]!=undefined){
						...
						if(flist[i].fundReviewImg != null && flist[i].fundReviewImg != ''){
							feedScrollHtml +="<p class='review_photo'><img alt='리뷰이미지'  src=\"img/fundingMainImg/"+flist[i].fundReviewImg+"\"  style='width: 800px;height: 400px;'></p>";					
						}
					...
					for(var i=0; i<StoreEndNum; i++){
						if(flist.length> StoreEndNum || flist[i]!=undefined){
							...
							if(flist[i].storeReviewImg != null && flist[i].storeReviewImg != ''){
								feedScrollHtml +="<p class='review_photo'><img alt='리뷰이미지' src='imgs/"+flist[i].storeReviewImg+"' style='width: 800px;height: 400px;'></p>";					
							}
		...
		<div class="content" id="feedajaxinsert">
			...
			<c:set var="i" value="${status.index}" />
					...
					<c:if test="${flist[i].fundReviewImg ne '' && not empty flist[i].fundReviewImg}">				
						<p class="review_photo"><img alt="리뷰이미지" src="img/fundingMainImg/${flist[i].fundReviewImg }" style='width: 800px;height: 400px;'></p>						
					</c:if>
