...
function colum(profileid,memberId,category) {
	...
	$.ajax("profileColumAjax.do",{
		...
		if(list.length==0){
			if(category=="FUND"){
				profilehtml +="<div><p style='font-size: 1em;text-align: center;'>펀딩한 내역이 없습니다.</p></div>";
			}
			else{
				profilehtml +="<div><p style='font-size: 1em;text-align: center;'>찜한 내역이 없습니다.</p></div>";
			}
		}else{
			...
			if(data.fundlist.length > endnum){
				buttonhtml="<button type='button' class='btn btn-primary' onclick=\"more('"+flist.memberId+"','"+memberId+"','"+category+"')\">더보기</button>";
				buttonhtml+="<input type='hidden' name='addNum' value='"+addnum+"'>";
				buttonhtml+="<input type='hidden' name='endNum' value='"+endnum+"'>";
				$("#btnAjaxinsert").html(buttonhtml);
			}else{
				$("#btnAjaxinsert button").remove();
			}
...
<div>
	...
	<hr style="margin:0;">
	<div class="profile_8">
	 	<section class="profile_7" style="padding: 80px 30px 100px 0px;"> 
			<ul id="moreajaxinsert">
				<!-- S : 리워드 리스트 -->
