		...
		<div class="container" id="con1">
			<div class="board-main">
				<ul>
					<li>
						<div class="info">
							<h3 class="title">제목 : ${fundingQnaView.fundQnaTitle }</h3><hr>
							<div style="text-align: center;">
								<div class="writer">
									<span class="author">작성자 : ${fundingQnaView.memberId } </span><br><hr>
								</div>
								<div class="writer">
									<span class="created-at">작성일 : ${fn:substring(fundingQnaView.fundQnaDate,0,16) } </span>
								</div>
							</div>
						</div>
					</li>					
				...
				<ul>
					<li>
					<br>
						<c:if test="${empty fundingQnaRe}">
							<div class="info">
								<h3 class="title">아직 답변이 없습니다. </h3><hr>
							</div>
						</c:if>
						<c:if test="${not empty fundingQnaRe }">
							<div class="info">
								...
								<div style="text-align: center;">
									...
									<div class="writer">
										<span class="created-at">작성일 : ${fn:substring(fundingQnaRe.fundQnaReDate,0,16) } </span><hr>
									</div>
								</div>
							</div>
						</c:if>
						...
						<div class="QnaRe">
							<form action="insertQnaRe.do">
								<c:if test="${user.memberId == 'admin'  }">
									...
									<button type="submit" onclick="insertQnaRe_go()" class="insertQnaRe"><i class="fa fa-pencil"></i>&nbsp;작성</button>
									<input type="hidden" name="fundQnaIdx" value="${fundingQnaView.fundQnaIdx }">
									<input type="hidden" name="fundQnaTitle" value="${fundingQnaView.fundQnaTitle }">
									<input type="hidden" name="fundIdx" value="${fundingQnaView.fundIdx }"> 
								</c:if>
							</form>
						</div>
					</div>
			...
			<div class="row">
				<div class="col-12">
					<div class="row">
						...
						<c:if test="${fundingQnaView.memberId == user.memberId || user.memberId == 'admin'}">
						...
						<div class="col-md-4"  style="text-align:right">
							<div class="go_delete">
								<a style="color:white" id="delete_btn" href="deleteQnA.do?fundQnaIdx=${fundingQnaView.fundQnaIdx }&memberId=${user.memberId}&fundIdx=${productDetailView.fundIdx }" onclick="return confirm('삭제하시겠습니까?');">삭제하기</a>
							</div>
						</div>
					</c:if>
				</div>
