    ...
    <section class="best-selling-products-area mb-70">
        <div class="container">
            ...
            <div class="row justify-content-center">
           		<c:if test="${empty fundingPageBabylEndDate}">
           			<tr>
           				<td>아직 데이터가 없습니다^^;;</td>
           			</tr>
           		</c:if>
                <c:if test="${not empty fundingPageBabylEndDate }">
	                <c:forEach var="fundingPageBabylEndDate" items="${fundingPageBabylEndDate}">
			                <div class="col-9 col-sm-6 col-md-4 col-lg-4">
			                    <div class="single-product-area mb-30">
			                        <div class="product_image">
			                            <!-- Product Image -->
			                           <a href="productDetailView.do?fundIdx=${fundingPageBabylEndDate.fundIdx}&memberId=${user.memberId}&fundMakerId=${getBoardListAll.fundMakerId}">
			                           		<img class="normal_img" src="img/fundingMainImg/${fundingPageBabylEndDate.fundMainImg }">
			                            </a>
			                            
			                            <!-- Wishlist -->
			                            <c:if test="${fn:contains(bool,fundingPageBabylEndDate.fundIdx)}">
		                            		<div class="product_wishlist">
						                 		<a href="javascript:fundLikeUpdate(${fundingPageBabylEndDate.fundIdx })">
						                 		<i id="heart" class="fa-solid fa-heart-circle-minus"></i></a>
						         			</div>
			                            </c:if>
			                    		<c:if test="${not fn:contains(bool,fundingPageBabylEndDate.fundIdx)}">
			                    			<div class="product_wishlist">
						                 		<a href="javascript:fundLikeUpdate(${fundingPageBabylEndDate.fundIdx })">
			                        ...
			                        <div class="product_description">
			                       	 	<c:choose>
			                       	 		<c:when test ="${fn:length(fundingPageBabylEndDate.fundTitle) > 15 }">
			                           			<a href="productDetailView.do?fundIdx=${fundingPageBabylEndDate.fundIdx}&memberId=${user.memberId}&fundMakerId=${getBoardListAll.fundMakerId}">${fn:substring(fundingPageBabylEndDate.fundTitle,0,15) }...</a>
			                           		</c:when>
			                           		<c:otherwise>
			                           			<a href="productDetailView.do?fundIdx=${fundingPageBabylEndDate.fundIdx}&memberId=${user.memberId}&fundMakerId=${getBoardListAll.fundMakerId}">${fn:substring(fundingPageBabylEndDate.fundTitle,0,15) }</a>
			                           		</c:otherwise>
			                        ...
			                        <div class="progress mb-3">
		                            	<div class="progress-bar" role="progressbar" style="width: ${fundingPageBabylEndDate.fundTotalPricePercent }%; background-color:#00c4c4;" aria-valuenow="${fundingPageBabylEndDate.fundTotalPricePercent }" aria-valuemin="0" aria-valuemax="100"></div>
		                       	 	</div>
		                       	 	
		                            <p class="price mb-2" style="font-weight:bold">
		                            	<b style="font-size:17px; color: #00c4c4;">${fundingPageBabylEndDate.fundTotalPricePercent } % </b><b>·  ${fundingPageBabylEndDate.fundPrice} 원 펀딩</b>
	                            		<c:choose >
			                            	<c:when test="${fundingPageBabylEndDate.fundDday > 10 }">
					                       		<b class="product-price"style="float: right; font-size:17px;">${fundingPageBabylEndDate.fundDday } 일 남음</b>
				                       		</c:when>
				                            <c:otherwise>
					                       		<b class="title mb-2" style="color:red; float: right; font-size:17px;">${fundingPageBabylEndDate.fundDday } 일 남음</b>
				                       		</c:otherwise>
			                       		</c:choose>
	                            	</p>
			                    </div>
