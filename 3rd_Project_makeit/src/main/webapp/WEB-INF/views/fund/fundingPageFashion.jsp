    ...
    <section class="best-selling-products-area mb-70">
        <div class="container">
                   						...
                   						<ul >
                   							<li><a href="fundingPageAll.do">펀딩 홈</a>
		                                        <ul class="dropdown" style=" position: absolute; z-index: auto">
		                                            ...
		                                            <li><a href="fundingPageFashionRecent.do" style="  position: relative; z-index: 2;">최신순</a></li>
		                                        </ul>
                                   			</li>
                   						</ul>
                   					</div>
            ...
            <div class="row justify-content-center">
           		<c:if test="${empty getBoardListFashion}">
           			<tr>
           				<td>아직 데이터가 없습니다^^;;</td>
           			</tr>
           		</c:if>
                <c:if test="${not empty getBoardListFashion }">
	                <c:forEach var="getBoardListFashion" items="${getBoardListFashion}">
			                <div class="col-9 col-sm-6 col-md-4 col-lg-4">
			                    <div class="single-product-area mb-30">
			                        <div class="product_image">
			                            <!-- Product Image -->
			                           <a href="productDetailView.do?fundIdx=${getBoardListFashion.fundIdx}&memberId=${user.memberId}&fundMakerId=${getBoardListAll.fundMakerId}">
			                           		<img class="normal_img" src="img/fundingMainImg/${getBoardListFashion.fundMainImg }">
			                            </a>
			                            
			                            <!-- Wishlist -->
			                            <c:if test="${fn:contains(bool,getBoardListFashion.fundIdx)}">
		                            		<div class="product_wishlist">
						                 		<a href="javascript:fundLikeUpdate(${getBoardListFashion.fundIdx })">
						                 		<i id="heart" class="fa-solid fa-heart-circle-minus"></i></a>
						         			</div>
			                            </c:if>
			                    		<c:if test="${not fn:contains(bool,getBoardListFashion.fundIdx)}">
			                    			<div class="product_wishlist">
						                 		<a href="javascript:fundLikeUpdate(${getBoardListFashion.fundIdx })">
			                        ...
			                        <div class="product_description">
			                       	 	<c:choose>
			                       	 		<c:when test ="${fn:length(getBoardListFashion.fundTitle) > 15 }">
			                           			<a href="productDetailView.do?fundIdx=${getBoardListFashion.fundIdx}&memberId=${user.memberId}&fundMakerId=${getBoardListAll.fundMakerId}">${fn:substring(getBoardListFashion.fundTitle,0,15) }...</a>
			                           		</c:when>
			                           		<c:otherwise>
			                           			<a href="productDetailView.do?fundIdx=${getBoardListFashion.fundIdx}&memberId=${user.memberId}&fundMakerId=${getBoardListAll.fundMakerId}">${fn:substring(getBoardListFashion.fundTitle,0,15) }</a>
			                           		</c:otherwise>
			                         ...
			                         <div class="progress mb-3">
		                            	<div class="progress-bar" role="progressbar" style="width: ${getBoardListFashion.fundTotalPricePercent }%; background-color:#00c4c4;" aria-valuenow="${getBoardListFashion.fundTotalPricePercent }" aria-valuemin="0" aria-valuemax="100"></div>
		                       	 	</div>
		                       	 	
		                            <p class="price mb-2" style="font-weight:bold">
		                            	<b style="font-size:17px; color: #00c4c4;">${getBoardListFashion.fundTotalPricePercent } % </b><b>·  ${getBoardListFashion.fundPrice} 원 펀딩</b>
	                            		<c:choose >
			                            	<c:when test="${getBoardListFashion.fundDday > 10 }">
					                       		<b class="product-price"style="float: right; font-size:17px;">${getBoardListFashion.fundDday } 일 남음</b>
				                       		</c:when>
				                            <c:otherwise>
					                       		<b class="title mb-2" style="color:red; float: right; font-size:17px;">${getBoardListFashion.fundDday } 일 남음</b>
				                       		</c:otherwise>
			                       		</c:choose>
	                            	</p>
			                    </div>
			...
			<div class="shop_pagination_area mt-30">
				<nav aria-label="Page navigation">
					<ul class="pagination pagination-sm justify-content-center"  style="display: flex;" id="ajaxpageinsert" >
						...
						<c:if test="${page.beginPage!=1 }">
							<li class="page-item">
								<a class="page-link" href="fundingPageFashion.do?cPage=${page.beginPage-1}"><i class="fa fa-angle-left" aria-hidden="true"></i></a>
							</li>
						</c:if>
						...
						<c:forEach var="pageNo" begin="${page.beginPage }" end="${page.endPage }">
							...
							<c:if test="${pageNo != page.nowPage }">
								<li class="page-item">
									<a class="page-link" href="fundingPageFashion.do?cPage=${pageNo }">${pageNo }</a>
								</li>
							</c:if>
						</c:forEach>
						...
						<li class="page-item"> 
							<a class="page-link" href="fundingPageFashion.do?cPage=${page.endPage +1 }"><i class="fa fa-angle-right" aria-hidden="true"></i></a>
						</li>
						</c:if>
						<c:if test="${page.endPage >= page.totalPage  }">
							<li class="page-item">
