    ...
    <section class="best-selling-products-area mb-70">
        <div class="container">
            ...
            <div class="row justify-content-center">
           		<c:if test="${empty fundingPageFashionFundPrice}">
           			<tr>
           				<td>아직 데이터가 없습니다^^;;</td>
           			</tr>
           		</c:if>
                <c:if test="${not empty fundingPageFashionFundPrice }">
	                <c:forEach var="fundingPageFashionFundPrice" items="${fundingPageFashionFundPrice}">
			                <div class="col-9 col-sm-6 col-md-4 col-lg-4">
			                    <div class="single-product-area mb-30">
			                        <div class="product_image">
			                            <!-- Product Image -->
			                           <a href="productDetailView.do?fundIdx=${fundingPageFashionFundPrice.fundIdx}&memberId=${user.memberId}&fundMakerId=${getBoardListAll.fundMakerId}">
			                           		<img class="normal_img" src="img/fundingMainImg/${fundingPageFashionFundPrice.fundMainImg }">
			                            </a>
			                            
			                            <!-- Wishlist -->
			                            <c:if test="${fn:contains(bool,fundingPageFashionFundPrice.fundIdx)}">
		                            		<div class="product_wishlist">
						                 		<a href="javascript:fundLikeUpdate(${fundingPageFashionFundPrice.fundIdx })">
						                 		<i id="heart" class="fa-solid fa-heart-circle-minus"></i></a>
						         			</div>
			                            </c:if>
			                    		<c:if test="${not fn:contains(bool,fundingPageFashionFundPrice.fundIdx)}">
			                    			<div class="product_wishlist">
						                 		<a href="javascript:fundLikeUpdate(${fundingPageFashionFundPrice.fundIdx })">
			                        ...
			                        <div class="product_description">
			                       	 	<c:choose>
			                       	 		<c:when test ="${fn:length(fundingPageFashionFundPrice.fundTitle) > 15 }">
			                           			<a href="productDetailView.do?fundIdx=${fundingPageFashionFundPrice.fundIdx}&memberId=${user.memberId}&fundMakerId=${getBoardListAll.fundMakerId}">${fn:substring(fundingPageFashionFundPrice.fundTitle,0,15) }...</a>
			                           		</c:when>
			                           		<c:otherwise>
			                           			<a href="productDetailView.do?fundIdx=${fundingPageFashionFundPrice.fundIdx}&memberId=${user.memberId}&fundMakerId=${getBoardListAll.fundMakerId}">${fn:substring(fundingPageFashionFundPrice.fundTitle,0,15) }</a>
			                           		</c:otherwise>
			                         ...
			                         <div class="progress mb-3">
		                            	<div class="progress-bar" role="progressbar" style="width: ${fundingPageFashionFundPrice.fundTotalPricePercent }%; background-color:#00c4c4;" aria-valuenow="${fundingPageFashionFundPrice.fundTotalPricePercent }" aria-valuemin="0" aria-valuemax="100"></div>
		                       	 	</div>
		                       	 	
		                            <p class="price mb-2" style="font-weight:bold">
		                            	<b style="font-size:17px; color: #00c4c4;">${fundingPageFashionFundPrice.fundTotalPricePercent } % </b><b>·  ${fundingPageFashionFundPrice.fundPrice} 원 펀딩</b>
	                            		<c:choose >
			                            	<c:when test="${getBoardListFashion.fundDday > 10 }">
					                       		<b class="product-price"style="float: right; font-size:17px;">${fundingPageFashionFundPrice.fundDday } 일 남음</b>
				                       		</c:when>
				                            <c:otherwise>
					                       		<b class="title mb-2" style="color:red; float: right; font-size:17px;">${fundingPageFashionFundPrice.fundDday } 일 남음</b>
				                       		</c:otherwise>
			                       		</c:choose>
	                            	</p>
			                    </div>
